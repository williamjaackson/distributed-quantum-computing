...
/// which, and what the control masks are all come from `planGate` in Rust — and
...
function handle(req: ShardReq): { data: unknown; transfer?: Transferable[] } {
  ...
  switch (req.kind) {
    ...
    case 'importApply': {
      refreshViews();
      const [start, end] = blockRange(req.block);
      scratchView.set(new Float64Array(req.buffer, 0, end - start));
      shard.applyPair(
        req.base,
        new Float64Array(req.params),
        req.block,
    ...
    case 'importDot': {
      ...
      const [start, end] = blockRange(req.block);
      scratchView.set(new Float64Array(req.buffer, 0, end - start));
      return { data: Array.from(shard.dotScratch(req.block)) };
    }

    case 'mass':
      return { data: shard.probabilityMass() };

    case 'localReducedOne':
      return { data: Array.from(shard.localReducedOne(req.qubit)) };

    case 'localTop': {
      const t = shard.localTopAmplitudesFlat(req.k);
      return { data: t, transfer: [t.buffer] };
    }

    case 'collapseLocal':
      shard.collapseLocal(req.qubit, req.outcome, req.scale);
      return { data: null };
    ...
    case 'amplitudes': {
      refreshViews();
      const out = new Float64Array(sliceSlots);
      out.set(ampsView);
      return { data: out, transfer: [out.buffer] };
    }
