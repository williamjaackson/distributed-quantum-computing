...
//
// Why this exists: mode B (qubit-expansion) moves exchange blocks up to
// BLOCK_AMPS * 16 bytes (64 MiB, see engine/src/shard.rs) between two peers.
// Browsers cap a single data-channel message well below that — the safe,
// portable ceiling across Chrome/Firefox/Safari is far smaller than the
// ~256 KiB some engines advertise — so every transfer gets split into fixed
// chunks with a small binary header and reassembled on the other end.
//
// A header (not just channel ordering) is load-bearing here: a peer may have
// several logical transfers in flight on the same 'bulk' channel at once
// (e.g. it is mid-exchange with two different partners for two different
// shard pairs), so chunks need enough identity to be reassembled correctly
// even if two transfers interleave.
 ...
 * long as the transfer is in flight — the caller picks it (e.g. a per-peer
 ...
 */
export function splitIntoChunks(transferId, buffer, chunkPayloadBytes = CHUNK_PAYLOAD_BYTES) {
...
export class ChunkReassembler {
  #pending = new Map(); // transferId -> { total, received: number, parts: Array<Uint8Array|undefined> }

  /**
   * Feed one chunk (an ArrayBuffer as received from the data channel).
   * Returns `{ transferId, buffer: ArrayBuffer }` when this chunk completes
   * its transfer, otherwise `null`.
   */
  receive(chunkArrayBuffer) {
    ...
    const totalBytes = entry.parts.reduce((sum, p) => sum + p.byteLength, 0);
