...
export const coin: Program = {
  ...
  *build(v): Iterable<Step> {
  ...
  outputs: ({ nQubits, bits, finished }) => {
    const measured = Array.from({ length: nQubits }, (_, q) => bits[`c${q}`]).filter(
      (b) => b !== undefined,
    ) as number[];
    if (measured.length === 0) {
      return [{ label: 'Outcome', value: finished ? 'not measured' : 'pending', hero: true }];
    }
    const heads = measured.filter((b) => b === 0).length;
    return [
      {
        label: 'Flips',
      ...
      { label: 'Heads', value: `${heads} of ${measured.length}` },
    ];
  },
};
